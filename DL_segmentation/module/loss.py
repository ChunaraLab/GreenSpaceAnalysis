...
def softmax_focalloss(y_pred, y_true, ignore_index=-1, gamma=2.0, normalize=False):
    ...
    Returns:
    """
    #losses = F.cross_entropy(y_pred, y_true, weight=0.5*torch.tensor([0.2, 1, 2, 3]).cuda(), ignore_index=ignore_index, reduction='none')
...
class SegmentationLoss(nn.Module):
    ...
    def forward(self, y_pred, y_true: torch.Tensor):
        ...
        if 'ce' in self.loss_config:
            #loss_dict['ce_loss'] = F.cross_entropy(y_pred, y_true.long(), weight=torch.tensor([0.24, 1., 2.849]).cuda() , ignore_index=-1)
            loss_dict['ce_loss'] = F.cross_entropy(y_pred, y_true.long(), ignore_index=-1)
        if 'fcloss' in self.loss_config:
            loss_dict['fc_loss'] = softmax_focalloss(y_pred, y_true.long(), gamma=self.loss_config.fcloss.gamma, normalize=True)
